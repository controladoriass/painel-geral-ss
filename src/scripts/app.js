...
(function(){
  ...
  async function renderFormularios(){
    // METAS (aba Comercial) — exibe todos os 5 tipos de meta que o form coleta
    const metas = await carregar('form_metas');
    if(metas && metas.metas && metas.metas.length && $('#com-metas-panel')){
      const rows = metas.metas.map(m => {
        const subs = [];
        if(m.meta_propostas_qtd) subs.push(`${m.meta_propostas_qtd} propostas`);
        if(m.meta_propostas_valor) subs.push(fmtBRLk(m.meta_propostas_valor));
        if(m.meta_assessorias) subs.push(`${m.meta_assessorias} assess.`);
        if(m.meta_ligacoes) subs.push(`${m.meta_ligacoes} lig.`);
        if(m.meta_leads) subs.push(`${m.meta_leads} leads`);
        const sub = subs.join(' · ') || '—';
        return `<div class="hbar-row">
          <div class="hb-name" title="${m.vendedor}">${m.vendedor}</div>
          <div class="hbar-track"><div class="hbar-fill gold" style="width:100%"></div></div>
          <div class="hb-val">${fmtNum(m.meta_propostas_qtd||m.meta_assessorias||0)}<small>${m.referencia||m.periodo||''} · ${sub}</small></div>
        </div>`;
      }).join('');
    ...
    if(atv && atv.por_vendedor && atv.por_vendedor.length && $('#com-atividade-panel')){
      ...
      $('#com-atividade-panel').innerHTML = `<div class="panel-label">Atividade acumulada por responsável</div>
        <div class="hbar-list">${rows}</div>
    ...
    // TIMES (aba Produção) — usa por_area_total se disponível (soma principal+secundárias)
    const times = await carregar('form_times');
    if(times && $('#prod-times-panel')){
      const fonte = times.por_area_total && Object.keys(times.por_area_total).length
        ? times.por_area_total
        : (times.por_time || {});
      const usaSecundarias = !!times.por_area_total;
      if(Object.keys(fonte).length){
        ...
        const maxAdvs = Math.max(...entradas.map(e => e[1].length)) || 1;
        const rows = entradas.map(([area, advs]) => `<div class="hbar-row">
          <div class="hb-name" title="${area}">${area}</div>
          <div class="hbar-track"><div class="hbar-fill gold" style="width:${Math.round(advs.length/maxAdvs*100)}%"></div></div>
          <div class="hb-val">${advs.length}<small>${advs.slice(0,3).join(', ')}${advs.length>3?'…':''}</small></div>
        </div>`).join('');
        const nota = usaSecundarias ? 'inclui áreas secundárias' : 'só área principal';
        $('#prod-times-panel').innerHTML = `<div class="panel-label">Advogados por área (${total} ativos · ${nota})</div>
          <div class="hbar-list">${rows}</div>
    ...
    // REAJUSTES (aba Financeiro) — mostra também nova vigência quando informada
    ...
    if(rea && rea.reajustes && rea.reajustes.length && $('#fin-reajustes-panel')){
      const rows = rea.reajustes.slice(0,15).map(r => {
        ...
        const extras = [];
        extras.push(`${fmtBRLk(r.valor_anterior)} → ${fmtBRLk(r.valor_novo)}`);
        extras.push(fmtData(r.data));
        if(r.nova_data_final) extras.push(`nova vig. ${fmtData(r.nova_data_final)}`);
        if(r.indice) extras.push(r.indice);
        return `<div class="hbar-row">
          ...
          <div class="hbar-track"><div class="hbar-fill gold" style="width:100%"></div></div>
          <div class="hb-val" style="color:${cor}">${sinal}${(r.variacao_pct||0).toFixed(2)}%<small style="color:var(--muted)">${extras.join(' · ')}</small></div>
        </div>`;
      }).join('');
