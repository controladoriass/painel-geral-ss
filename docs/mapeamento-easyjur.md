...
**🚨 BUG CRÍTICO DA API DE DESPESAS (descoberto 31/08):** o filtro `data_pagamento_inicio/fim` de `list_despesas` **NÃO é respeitado pelo endpoint**. Ao pedir despesas pagas de 2026, o EasyJur devolve **7.593 registros — mas só 1.593 são realmente de 2026** (o resto vem de 2023 e 2024 misturados). Somar bruto **infla o valor ~13x** (R$ 93,9M vs R$ 6,9M reais). **REGRA:** toda coleta de despesas por data_pagamento DEVE filtrar em Python por `datas.pagamento[:4]==ano` depois de baixar. Testar se o mesmo bug ocorre em list_receitas por precaução na próxima coleta.
